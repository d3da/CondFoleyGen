...
class InfoNCELoss(pl.LightningModule):
    ...
    def forward(self, m_emb, label_emb, classes, label_mask):
        ...
        pos_similarities = sim_matrix.mul(classes_one_hot).sum(dim=1)  # B
        neg_classes = 1 - classes_one_hot  # B x L
        n_neg_classes = neg_classes.sum(dim=1)
        neg_similarities = sim_matrix.mul(neg_classes).sum(dim=1).div(n_neg_classes) # B

        return loss.mean(), dict(pos_similarities=pos_similarities.mean(),
                                 neg_similarities=neg_similarities.mean())
