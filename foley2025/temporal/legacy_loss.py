...
class TCCLoss(pl.LightningModule):
    ...
    def forward(self, batched_u, batched_v):
        ...
        for u, v in zip(batched_u, batched_v):
            total += self.tcc_loss(u, v)

        return total


    def tcc_loss(self, *sequences):
        ...
        for i, primary in enumerate(sequences):
            ...
            for j, secondary in enumerate(sequences):
                ...
                ALPHA = self.normalizer(nn.functional.cosine_similarity(primary.unsqueeze(1), secondary.unsqueeze(0), dim=2).T / self.softmax_temp).T
                SNN = ALPHA @ secondary
                #########BETA = self.normalizer(-torch.cdist(SNN, primary, p=2).T / self.softmax_temp).T
                BETA = self.normalizer(nn.functional.cosine_similarity(SNN.unsqueeze(1), primary.unsqueeze(0), dim=2).T / self.softmax_temp).T
                mus = BETA @ idx_range
